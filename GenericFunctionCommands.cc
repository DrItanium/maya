...
static void PrintGenericCall(const Environment::Ptr&, const char *, Defgeneric *);
static bool EvaluateGenericCall(const Environment::Ptr&, Defgeneric *, UDFValue *);
static void DecrementGenericBusyCount(const Environment::Ptr&, Defgeneric *);
static void IncrementGenericBusyCount(const Environment::Ptr&, Defgeneric *);
static void DeallocateDefgenericData(const Environment::Ptr&);

static void DestroyDefgenericAction(const Environment::Ptr&, ConstructHeader *, void *);

static void SaveDefgenerics(const Environment::Ptr&, Defmodule *, const char *, void *);
static void SaveDefmethods(const Environment::Ptr&, Defmodule *, const char *, void *);
static void SaveDefmethodsForDefgeneric(const Environment::Ptr&, ConstructHeader *, void *);
static void RemoveDefgenericMethod(const Environment::Ptr&, Defgeneric *, unsigned short);

#if DEBUGGING_FUNCTIONS
static unsigned short ListMethodsForGeneric(const Environment::Ptr&, const char *, Defgeneric *);
static bool DefgenericWatchAccess(const Environment::Ptr&, int, bool, Expression *);
static bool DefgenericWatchPrint(const Environment::Ptr&, const char *, int, Expression *);
static bool DefmethodWatchAccess(const Environment::Ptr&, int, bool, Expression *);
static bool DefmethodWatchPrint(const Environment::Ptr&, const char *, int, Expression *);
static bool DefmethodWatchSupport(const Environment::Ptr&, const char *, const char *, bool,
                                  void (*)(const Environment::Ptr&, const char *, Defgeneric *, unsigned short),
                                  void (*)(Defgeneric *, unsigned short, bool),
                                  Expression *);
static void PrintMethodWatchFlag(const Environment::Ptr&, const char *, Defgeneric *, unsigned short);
#endif
...
void SetupGenericFunctions(
    ...
    AddClearReadyFunction(theEnv, "defgeneric", ClearDefgenericsReady, 0, nullptr);
    ...
    /* ================================================================
       ...
       ================================================================ */
    AddSaveFunction(theEnv, "defgeneric", SaveDefgenerics, 1000, nullptr);
    AddSaveFunction(theEnv, "defmethod", SaveDefmethods, -1000, nullptr);
    AddUDF(theEnv, "undefgeneric", "v", 1, 1, "y", UndefgenericCommand);
    AddUDF(theEnv, "undefmethod", "v", 2, 2, "*;y;ly", UndefmethodCommand);

    AddUDF(theEnv, "call-next-method", "*", 0, 0, nullptr, CallNextMethod);
    FuncSeqOvlFlags(theEnv, "call-next-method", true, false);
    AddUDF(theEnv, "call-specific-method", "*", 2, UNBOUNDED, "*;y;l", CallSpecificMethod);
    FuncSeqOvlFlags(theEnv, "call-specific-method", true, false);
    AddUDF(theEnv, "override-next-method", "*", 0, UNBOUNDED, nullptr, OverrideNextMethod);
    FuncSeqOvlFlags(theEnv, "override-next-method", true, false);
    AddUDF(theEnv, "next-methodp", "b", 0, 0, nullptr, NextMethodPCommand);
    FuncSeqOvlFlags(theEnv, "next-methodp", true, false);

    AddUDF(theEnv, "(gnrc-current-arg)", "*", 0, UNBOUNDED, nullptr, GetGenericCurrentArgument);

#if DEBUGGING_FUNCTIONS
    AddUDF(theEnv, "ppdefgeneric", "vs", 1, 2, ";y;ldsyn", PPDefgenericCommand);
    AddUDF(theEnv, "list-defgenerics", "v", 0, 1, "y", ListDefgenericsCommand);
    AddUDF(theEnv, "ppdefmethod", "v", 2, 3, "*;y;l;ldsyn", PPDefmethodCommand);
    AddUDF(theEnv, "list-defmethods", "v", 0, 1, "y", ListDefmethodsCommand);
    AddUDF(theEnv, "preview-generic", "v", 1, UNBOUNDED, "*;y", PreviewGeneric);
#endif

    AddUDF(theEnv, "get-defgeneric-list", "m", 0, 1, "y", GetDefgenericListFunction);
    AddUDF(theEnv, "get-defmethod-list", "m", 0, 1, "y", GetDefmethodListCommand);
    AddUDF(theEnv, "get-method-restrictions", "m", 2, 2, "l;y", GetMethodRestrictionsCommand);
    AddUDF(theEnv, "defgeneric-module", "y", 1, 1, "y", GetDefgenericModuleCommand);

    AddUDF(theEnv, "type", "*", 1, 1, "*", ClassCommand);
...
static void DeallocateDefgenericData(
    ...
    DoForAllConstructs(theEnv,
                       ...
                       DefgenericData(theEnv)->DefgenericModuleIndex, false, nullptr);
...
#pragma unused(buffer)
    ...
    if (theDefgeneric == nullptr) return;
...
/***************************************************
  ...
  RETURNS      : The defgeneric (nullptr if not found)
...
/***********************************************************
  ...
  RETURNS      : The address of the next generic function
                   (nullptr if none)
  SIDE EFFECTS : None
  NOTES        : If ptr == nullptr, the first generic function
                    is returned.
 ***********************************************************/
...
unsigned short GetNextDefmethod(
    ...
    if (theIndex == 0) {
        if (theDefgeneric->methods != nullptr) { return theDefgeneric->methods[0].index; }
...
bool DefgenericIsDeletable(
        Defgeneric *theDefgeneric) {
    const Environment::Ptr&theEnv = theDefgeneric->header.env;
...
void UndefmethodCommand(
    ...
    if ((gfunc == nullptr) ? (strcmp(theArg.lexemeValue->contents, "*") != 0) : false) {
...
/**************************************************************
  ...
  DESCRIPTION  : Deletes all methods for a generic function
  INPUTS       : The generic-function address (nullptr for all)
...
/**************************************************************
  NAME         : Undefmethod
  DESCRIPTION  : Deletes one method for a generic function
  INPUTS       : 1) Address of generic function (can be nullptr)
...
void PPDefmethodCommand(
    ...
    if (strcmp(logicalName, "nil") == 0) {
        if (gfunc->methods[gi].header.ppForm != nullptr) { returnValue->lexemeValue = CreateString(theEnv, gfunc->methods[gi].header.ppForm); }
        else { returnValue->lexemeValue = CreateString(theEnv, ""); }
    } else {
        if (gfunc->methods[gi].header.ppForm != nullptr)
...
void ListDefmethodsCommand(
    ...
    if (!UDFHasNextArgument(context)) { ListDefmethods(theEnv, STDOUT, nullptr); }
    else {
        if (!UDFFirstArgument(context, SYMBOL_BIT, &theArg)) return;

        gfunc = CheckGenericExists(theEnv, "list-defmethods", theArg.lexemeValue->contents);
        if (gfunc != nullptr) { ListDefmethods(theEnv, STDOUT, gfunc); }
    }
}
...
/******************************************************
  ...
  INPUTS       : 1) The logical name of the output
                 2) Generic function to list methods for
                    (nullptr means list all methods)
  RETURNS      : Nothing useful
  SIDE EFFECTS : None
  NOTES        : None
 ******************************************************/
...
/***********************************************************
  ...
  DESCRIPTION  : Groups indices of all methdos for a generic
                 ...
                 (nullptr means get methods for all generics)
...
void GetDefmethodListCommand(
    ...
    if (!UDFHasNextArgument(context)) {
        GetDefmethodList(theEnv, &result, nullptr);
...
void GetDefmethodList(
    ...
    if (theDefgeneric != nullptr) {
        ...
        SetNextDefgeneric(theDefgeneric, nullptr);
    } else {
        gfunc = GetNextDefgeneric(theEnv, nullptr);
        svnxt = (gfunc != nullptr) ? GetNextDefgeneric(theEnv, gfunc) : nullptr;
    }
    count = 0;
    for (svg = gfunc;
         gfunc != nullptr;
...
void GetMethodRestrictions(
    ...
    for (i = 0; i < meth->restrictionCount; i++) {
        ...
        theList->contents[roffset++].lexemeValue = (rptr->query != nullptr) ? TrueSymbol(theEnv) : FalseSymbol(theEnv);
...
static void SaveDefmethodsForDefgeneric(
    ...
    for (i = 0; i < gfunc->mcnt; i++) {
        if (gfunc->methods[i].header.ppForm != nullptr) {
...
#endif
    if (newState)
        return (DefmethodWatchSupport(theEnv, "watch", nullptr, newState, nullptr, DefmethodSetWatch, argExprs));
    else
        return (DefmethodWatchSupport(theEnv, "unwatch", nullptr, newState, nullptr, DefmethodSetWatch, argExprs));
}
...
#pragma unused(code)
    ...
    return (DefmethodWatchSupport(theEnv, "list-watch-items", logName, 0,
                                  PrintMethodWatchFlag, nullptr, argExprs));
}
...
/*******************************************************
  ...
  INPUTS       : 1) The calling function name
                 2) The logical output name for displays
                    (can be nullptr)
                 3) The new set state
                 4) The print function (can be nullptr)
                 5) The trace function (can be nullptr)
...
static bool DefmethodWatchSupport(
        ...
        const char *logName,
        bool newState,
        void (*printFunc)(const Environment::Ptr&, const char *, Defgeneric *, unsigned short),
    ...
    if (argExprs == nullptr) {
        ...
        while (theModule != nullptr) {
            ...
            theGeneric = GetNextDefgeneric(theEnv, nullptr);
            while (theGeneric != nullptr) {
    ...
    while (argExprs != nullptr) {
        ...
        if ((genericName.header->type != SYMBOL_TYPE) ? true :
            ((theGeneric =
                      LookupDefgenericByMdlOrScope(theEnv, genericName.lexemeValue->contents)) == nullptr)) {
