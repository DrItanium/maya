...
void AttachGoal(
  ...
  bool queue)
  {
   ...
   if (queue)
     { AddToGoalQueue(theGoal,ASSERT_ACTION); }
...
void UpdateGoalSupport(
  ...
  struct partialMatch *goalParent)
  {
   ...
   if (theGoal->supportCount != 0) return;

   /*=================================*/
   /* Queue a retraction of the goal. */
   /*=================================*/
   
   AddToGoalQueue(theGoal,RETRACT_ACTION);
  }
      ...
      if (newGoal->theProposition.contents[i].value == VoidConstant(theEnv))
        {
         if (! cardinalityArray[i])
           {
            newGoal->theProposition.contents[i].integerValue = CreateUQV(theEnv,uqvIndex++);
           }
         else
         ...
         for (j = 0; j < theLength; j++)
           {
            if (theMultifield->contents[j].value == NULL)
              { theMultifield->contents[j].integerValue = CreateUQV(theEnv,uqvIndex++); }
           }
        }
     }
         ...
         if (theNode->networkTest->nextArg->type == FACT_SLOT_LENGTH)
           {
            ...
            hack = (struct factCheckLengthPNCall *) ((CLIPSBitMap *) theNode->networkTest->nextArg->value)->contents;
